...
import { 
  ...
  Crown, PawPrint, Radar, Lock, Folders
} from "lucide-react";
import { 
...
const dict = {
  en: {
    ...
    th_vol: "Volume",
    th_pnl: "Profit (PnL)",
    searching_liq: "Searching for institutional liquidity...",
  ...
  id: {
    ...
    th_vol: "Volume",
    th_pnl: "Profit (PnL)",
    searching_liq: "Mencari celah likuiditas...",
...
export default function Dashboard() {
  ...
  // STATE DATA ISOLATION (CASCADING)
  ...
  // ============================================================================
  // 1. DATA FETCHING (MASTER & CLUSTER ISOLATION)
  // ============================================================================
  useEffect(() => {
    ...
    if (role === 'super_admin') {
      ...
      unsubs.push(onValue(gRef, s => {
        const d = s.val() || {};
        setGroupsList([{ id: 'ALL', name: 'ALL GROUPS (GLOBAL)' }, ...Object.keys(d).map(k => ({ id: k, ...d[k] }))]);
      }));
      const aRef = ref(db, 'account_data');
      unsubs.push(onValue(aRef, s => {
        setAllAccountsData(s.val() || {});
        setIsLoading(false);
      }));
    } 
    // 🟢 ADMIN: Tarik HANYA Grup yang Didelegasikan, lalu tarik akun di dalamnya
    else if (role === 'admin') {
      const mRef = ref(db, `users/${user.uid}/managed_groups`);
      unsubs.push(onValue(mRef, mSnap => {
        const managed = mSnap.val() || {};
        const allowedGroups = Object.keys(managed).filter(k => managed[k]);
        
        if (allowedGroups.length === 0) {
          setGroupsList([]); setIsLoading(false); return;
        }

        const gRef = ref(db, 'groups');
        unsubs.push(onValue(gRef, gSnap => {
          const allG = gSnap.val() || {};
          const myG = allowedGroups.map(id => ({ id, ...allG[id] })).filter(g => g.name);
          setGroupsList(myG);
          
          let allowedAccs = [];
          myG.forEach(g => { if(g.accounts) allowedAccs.push(...Object.keys(g.accounts)) });
          const uniqueAccs = [...new Set(allowedAccs)];

          uniqueAccs.forEach(acc => {
             const accRef = ref(db, `account_data/${acc}`);
             unsubs.push(onValue(accRef, aSnap => {
               setAllAccountsData(prev => ({ ...prev, [acc]: aSnap.val() || {} }));
             }));
          });
          setIsLoading(false);
        }));
    ...
    // 🟢 INVESTOR: Tidak kenal Grup, hanya tarik Akun spesifik miliknya
    else if (role === 'investor') {
      const uRef = ref(db, `users/${user.uid}/owned_accounts`);
      unsubs.push(onValue(uRef, snap => {
         const owned = snap.val() || {};
         const allowedIds = Object.keys(owned).filter(k => owned[k]);
         setGroupsList([]); // Investor tidak ada UI Grup
         
         if (allowedIds.length === 0) {
            setAccountsList([]); setIsLoading(false); return;
         }
  ...
  // ============================================================================
  // Atur default selected Group (jika Admin/Super Admin)
  useEffect(() => {
    if ((role === 'super_admin' || role === 'admin') && groupsList.length > 0) {
      if (!selectedGroupId || !groupsList.find(g => g.id === selectedGroupId)) {
  ...
  useEffect(() => {
    ...
    // Auto-select akun pertama jika yang dipilih sebelumnya sudah tidak valid di grup ini
    if (newAccList.length > 0 && !newAccList.includes(selectedAccountId)) {
      setSelectedAccountId(newAccList[0]);
    } else if (newAccList.length === 0) {
      setSelectedAccountId("");
    }
  }, [selectedGroupId, allAccountsData, role, groupsList, selectedAccountId]);
  ...
  useEffect(() => {
    if (botType === "NON_ML") {
      setWsConnected(false); setTerminalLogs([]);
    } else {
      setTerminalLogs([{ time: getGMT8Time(), text: `SYSTEM BOOT: Connection request to Node...`, type: "SYSTEM" }]);
      setRobotState('SCANNING'); lastLogTimeRef.current = 0;
    }
  ...
  useEffect(() => {
    ...
    if (terminalData) {
      ...
      if (now - nodeTime < 25000) { if (!wsConnected) setWsConnected(true); } 
      else { if (wsConnected) setWsConnected(false); setRobotState('SYSTEM_ERROR'); }
  ...
  const getTerminalLogColor = (type) => {
    ...
    if (type === 'SCANNING_IDLE') return 'text-slate-500';
  ...
  // ==========================================
  // TAMPILAN RESTRIKSI (EMPTY STATES)
  // ==========================================
  ...
  if (isLoading) return <div className="flex h-screen items-center justify-center font-bold text-[var(--primary)] animate-pulse text-xl bg-[#030712]">Connecting to Server...</div>;
  
  // Jika Admin/Investor belum punya hak akses sama sekali
  if (role === 'investor' && accountsList.length === 0) return (
    <div className="flex flex-col items-center justify-center h-[80vh] text-center"><Lock size={64} className="text-red-500 opacity-50 mb-2" /><h2 className="text-xl font-black text-red-500 uppercase">{t.restricted_access}</h2><p className="text-sm font-bold text-gray-500">{t.contact_admin}</p></div>
  );
  if (role === 'admin' && groupsList.length === 0) return (
    <div className="flex flex-col items-center justify-center h-[80vh] text-center"><Folders size={64} className="text-purple-500 opacity-50 mb-2" /><h2 className="text-xl font-black text-purple-500 uppercase">{t.no_group_access}</h2><p className="text-sm font-bold text-gray-500">{t.contact_admin}</p></div>
  );
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 max-w-7xl mx-auto font-sans transition-colors duration-300">
      ...
      <style dangerouslySetInnerHTML={{__html: `
        @keyframes lightning-flash { 0%, 100% { opacity: 0.2; filter: brightness(1) drop-shadow(0 0 5px rgba(220,38,38,0.5)); box-shadow: 0 0 0 0 rgba(220,38,38,0); } 5% { opacity: 1; filter: brightness(2) drop-shadow(0 0 20px rgba(220,38,38,1)); box-shadow: 0 0 20px 5px rgba(220,38,38,0.8); } 10% { opacity: 0.1; filter: brightness(0.8); } 12% { opacity: 1; filter: brightness(2.5) drop-shadow(0 0 30px rgba(220,38,38,1)); box-shadow: 0 0 30px 10px rgba(220,38,38,1); } 15% { opacity: 0.2; } }
        .anim-lightning { animation: lightning-flash 1.5s infinite; border-radius: 50%; border: 2px solid rgba(220,38,38,0.5); width: 100%; height: 100%; position: absolute; }
        @keyframes god-shine { 0% { transform: translateY(-100%); opacity: 0; } 20%, 80% { opacity: 1; } 100% { transform: translateY(100%); opacity: 0; } }
        .anim-shine { background: linear-gradient(to bottom, transparent 0%, rgba(245, 158, 11, 0.6) 50%, transparent 100%); animation: god-shine 2.5s ease-in-out infinite; border-radius: 50%; width: 100%; height: 100%; position: absolute; }
        @keyframes scan-radar { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
        .anim-radar { background: conic-gradient(from 0deg, transparent 60%, rgba(16, 185, 129, 0.5) 90%, rgba(16, 185, 129, 1) 100%); animation: scan-radar 2s linear infinite; border-radius: 50%; width: 100%; height: 100%; position: absolute; }
      `}} />

      {/* HEADER PORTFOLIO & CASCADING DROPDOWNS */}
      <div className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-3xl p-6 md:p-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 shadow-sm relative overflow-hidden">
        ...
        <div className="z-10 w-full lg:w-auto">
          <h2 className="text-sm font-black text-[var(--muted-foreground)] uppercase tracking-widest flex items-center gap-2 mb-4"><Activity className="text-[var(--primary)]" size={16}/> {t.port_title}</h2>
          {selectedAccountId ? (
            <div className="flex flex-wrap items-center gap-4">
              <span className="text-4xl md:text-5xl font-black text-[var(--foreground)] tracking-tight">{formatCur(liveData.equity || 0)}</span>
              {Number(liveData.total_floating || 0) < 0 ? (
                  <span className="text-sm font-bold bg-red-500/10 text-red-500 px-3 py-1.5 rounded-lg flex items-center border border-red-500/20 shadow-sm"><TrendingDown size={16} className="mr-1.5"/> {t.floating} {formatCur(liveData.total_floating)}</span>
              ) : (
                  <span className="text-sm font-bold bg-green-500/10 text-green-500 px-3 py-1.5 rounded-lg flex items-center border border-green-500/20 shadow-sm"><TrendingUp size={16} className="mr-1.5"/> {t.floating} +{formatCur(liveData.total_floating)}</span>
              )}
            </div>
          ) : (
            <div className="text-2xl font-black text-[var(--muted-foreground)] tracking-tight">No Account Selected</div>
          )}
        </div>

        {/* Kontrol Kanan (Dropdowns) */}
        <div className="z-10 w-full lg:w-auto flex flex-col items-start lg:items-end gap-4 bg-[var(--muted)]/50 p-4 rounded-2xl border border-[var(--card-border)]">
          <div className="flex flex-wrap items-center gap-3 w-full justify-between lg:justify-end">
            {selectedAccountId && (
              <div className="flex items-center gap-1.5 bg-[var(--background)] px-3 py-1.5 rounded-lg border border-[var(--card-border)] shadow-sm">
                <span className="relative flex h-2 w-2"><span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${isEATrading ? 'bg-green-400' : 'bg-blue-400'}`}></span><span className={`relative inline-flex rounded-full h-2 w-2 ${isEATrading ? 'bg-green-500' : 'bg-blue-500'}`}></span></span>
                <span className={`text-[10px] font-bold uppercase tracking-wider ${isEATrading ? 'text-green-500' : 'text-blue-500'}`}>{isEATrading ? t.in_market : t.standby}</span>
              </div>
            )}
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full">
             <div className="flex flex-col min-w-[120px]">
                <span className="text-[10px] font-bold text-[var(--muted-foreground)] uppercase tracking-wider flex items-center gap-1"><Server size={10}/> {selectedAccountId ? brokerName : "---"}</span>
                <span className="text-xs font-bold text-[var(--foreground)] flex items-center gap-1 mt-0.5"><User size={12} className="text-[var(--primary)]"/> {selectedAccountId ? accountName : "---"}</span>
             </div>

             {/* DROPDOWN 1: PILIH CLUSTER (KHUSUS ADMIN/SUPER ADMIN) */}
             {role !== 'investor' && groupsList.length > 0 && (
               <div className="relative w-full sm:w-auto mt-2 sm:mt-0 min-w-[150px]">
                 <select 
                   value={selectedGroupId} 
                   onChange={(e) => setSelectedGroupId(e.target.value)}
                   className="appearance-none w-full bg-[var(--card-bg)] border border-purple-500/30 text-purple-500 text-xs font-bold rounded-xl pl-3 pr-8 py-2.5 outline-none focus:ring-2 focus:ring-purple-500 cursor-pointer shadow-sm"
                 >
                   {groupsList.map(g => (
                     <option key={g.id} value={g.id}>{g.name}</option>
                   ))}
                 </select>
                 <ChevronDown size={14} className="absolute right-3 top-3 text-purple-500 pointer-events-none" />
               </div>
             )}

             {/* DROPDOWN 2: PILIH AKUN */}
             <div className="relative w-full sm:w-auto mt-2 sm:mt-0 min-w-[150px]">
               <select 
          ...
          {selectedAccountId && <p className="text-[10px] text-[var(--muted-foreground)] font-medium flex items-center gap-1 mt-1"><Clock size={10} className="text-[var(--primary)]"/> {t.update} {formatTimeGMT8(liveData.last_update)}</p>}
        </div>
      </div>

      {/* RENDER KOSONG JIKA TIDAK ADA AKUN TERPILIH */}
      {!selectedAccountId ? (
        <div className="py-20 flex flex-col items-center justify-center bg-[var(--card-bg)] border border-[var(--card-border)] border-dashed rounded-3xl">
           <AlertTriangle size={48} className="text-[var(--muted-foreground)] opacity-50 mb-4" />
           <p className="text-lg font-bold text-[var(--muted-foreground)] uppercase tracking-widest">{t.no_account}</p>
        </div>
      ) : (
        <>
          {/* DYNAMIC AI COMMAND CENTER */}
          {botType !== "NON_ML" && (
            <div className={`border rounded-3xl p-4 md:p-6 grid grid-cols-1 md:grid-cols-3 gap-6 relative overflow-hidden transition-all duration-700 bg-[var(--card-bg)] ${theme.border} ${wsConnected ? theme.glow : 'shadow-none opacity-80'}`}>
              <div className={`absolute top-4 left-4 w-10 h-10 border-t-4 border-l-4 ${theme.brackets} ${wsConnected ? 'opacity-60' : 'opacity-30'} rounded-tl-xl pointer-events-none`}></div><div className={`absolute top-4 right-4 w-10 h-10 border-t-4 border-r-4 ${theme.brackets} ${wsConnected ? 'opacity-60' : 'opacity-30'} rounded-tr-xl pointer-events-none`}></div><div className={`absolute bottom-4 left-4 w-10 h-10 border-b-4 border-l-4 ${theme.brackets} ${wsConnected ? 'opacity-60' : 'opacity-30'} rounded-bl-xl pointer-events-none`}></div><div className={`absolute bottom-4 right-4 w-10 h-10 border-b-4 border-r-4 ${theme.brackets} ${wsConnected ? 'opacity-60' : 'opacity-30'} rounded-br-xl pointer-events-none`}></div>
              <div className="flex flex-col items-center justify-center text-center space-y-4 relative z-10">
                <div className={`w-32 h-32 flex-shrink-0 rounded-full flex items-center justify-center p-2 relative ${theme.bg} border-2 ${theme.border} overflow-hidden`}>
                  {robotState === 'SCANNING' && (<>{botType === 'GOD_MODE' && <div className="anim-shine"></div>}{botType === 'BEAST_MODE' && <div className="anim-lightning"></div>}{botType === 'ENIGMA_OTE' && <div className="anim-radar"></div>}</>)}
                  <div className="relative z-10 w-full h-full flex items-center justify-center">{robotState !== 'SCANNING' && animUrl[robotState] ? (<Lottie path={animUrl[robotState]} loop autoplay style={{width: '90%'}}/>) : (<BotIcon size={56} className={`${theme.accent} ${botType === 'BEAST_MODE' ? 'animate-pulse' : 'transition-colors duration-500'}`} />)}</div>
                </div>
                <div><h3 className={`text-xl font-black uppercase tracking-tighter flex items-center justify-center gap-2 drop-shadow-sm ${theme.accent}`}>{theme.name}</h3><p className="text-[10px] font-mono font-bold text-[var(--muted-foreground)] uppercase tracking-widest">{theme.vibe}</p></div>
                <div className="flex gap-4 bg-[var(--background)] px-4 py-2 rounded-xl border border-[var(--card-border)] shadow-inner"><span className="flex items-center gap-1 text-[10px] font-bold transition-colors"><Activity size={12} className={wsConnected ? "text-blue-500 animate-pulse" : "text-gray-500"}/> {wsConnected ? `${ping}ms` : '--'}</span><span className="flex items-center gap-1 text-[10px] font-bold"><Server size={12} className={theme.accent}/> {t.cloud_sync}</span></div>
              </div>
              <div className={`md:col-span-2 rounded-2xl border ${theme.border} p-4 font-mono text-[11px] sm:text-xs h-60 overflow-y-auto flex flex-col gap-2 relative shadow-inner group transition-colors duration-500 ${theme.termBg}`}>
                ...
                <div className={`sticky top-0 pb-2 border-b border-slate-300 dark:border-gray-800 mb-2 flex justify-between items-center z-10 backdrop-blur-md transition-colors duration-500 ${theme.termBg}/90`}><span className="text-slate-500 dark:text-gray-500 font-bold flex items-center gap-2 tracking-widest text-[10px] md:text-xs uppercase"><Terminal size={14} className="text-slate-400 dark:text-gray-400"/> {theme.exe}{wsConnected ? (<span className="hidden sm:inline-block ml-2 px-1.5 py-0.5 bg-green-500/10 border border-green-500/30 text-green-700 dark:text-green-500 rounded text-[8px] animate-pulse">{t.link_online}</span>) : (<span className="hidden sm:inline-block ml-2 px-1.5 py-0.5 bg-red-500/10 border border-red-500/30 text-red-700 dark:text-red-500 rounded text-[8px]">{t.link_offline}</span>)}</span><div className="flex gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-red-500/70"></div><div className="w-2.5 h-2.5 rounded-full bg-yellow-500/70"></div><div className="w-2.5 h-2.5 rounded-full bg-green-500/70"></div></div></div>
                <div className="z-10 flex flex-col gap-2 flex-grow scroll-smooth">
                  {terminalLogs.map((log, i) => (<div key={i} className="flex gap-3 animate-in fade-in slide-in-from-bottom-1 transition-colors duration-300"><span className="text-slate-400 dark:text-gray-600 shrink-0 font-bold">[{log.time}]</span><span className={`break-words ${getTerminalLogColor(log.type)}`}><span className="opacity-50 mr-1">{(log.type || '').includes('SCANNING') ? '>' : '>>'}</span> {log.text}</span></div>))}
          ...
          <div className="bg-[var(--muted)]/50 rounded-2xl p-5 border border-[var(--card-border)] flex flex-wrap gap-4 justify-between items-center shadow-sm">
            ...
            <div className="flex items-center gap-3 w-full md:w-auto border-t md:border-t-0 border-[var(--card-border)] pt-4 md:pt-0 mt-2 md:mt-0">
              <div><p className="text-[9px] font-bold text-[var(--muted-foreground)] uppercase tracking-widest md:text-right">{t.net_capital}</p><p className="font-black text-xl tracking-tight text-[var(--primary)]">{formatCur(Number(liveData.initial_deposit||0) + Number(liveData.additional_deposits||0) - Number(liveData.total_withdrawals||0))}</p></div>
            </div>
          </div>
              ...
              <div className="w-full h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={mockChartData} margin={{ top: 5, right: 0, left: -20, bottom: 0 }}>
                    <defs><linearGradient id="colorEquity" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#3b82f6" stopOpacity={0.3}/><stop offset="95%" stopColor="#3b82f6" stopOpacity={0.0}/></linearGradient></defs>
                ...
                {realDailyHistory.length > 0 ? (
                  realDailyHistory.map((day, idx) => (
                    <div key={idx} className="flex justify-between items-center p-3 rounded-xl bg-[var(--background)] border border-[var(--card-border)] hover:bg-[var(--muted)] transition-colors shadow-sm">
                      <div><p className="text-xs font-bold text-[var(--foreground)] uppercase">{day.date}</p><p className="text-[10px] text-[var(--muted-foreground)] flex items-center gap-1 mt-0.5"><BarChart3 size={10}/> {Number(day.lot).toFixed(1)} Vol</p></div>
                      <div className="text-right"><p className={`text-sm font-black ${day.profit >= 0 ? "text-green-500" : "text-red-500"}`}>{day.profit > 0 ? "+" : ""}{formatCur(day.profit)}</p><p className={`text-[9px] font-bold ${day.growth >= 0 ? "text-green-600 bg-green-500/10" : "text-red-600 bg-red-500/10"} inline-block px-1.5 py-0.5 rounded mt-0.5`}>{day.growth > 0 ? "+" : ""}{Number(day.growth).toFixed(1)}%</p></div>
                    </div>
                  ))
                ) : (<div className="text-center py-6 border border-[var(--card-border)] border-dashed rounded-xl"><p className="text-xs font-bold text-[var(--muted-foreground)]">{t.no_history}</p></div>)}
              </div>
            </div>
          </div>
