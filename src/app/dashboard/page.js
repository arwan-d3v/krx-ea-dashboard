...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // 🟢 LOGIKA SUPER ADMIN: Tarik SEMUA data akun tanpa terkecuali
    if (role === 'super_admin') {
      ...
      unsubAll = onValue(accountsRef, (snapshot) => {
        ...
        console.error("Firebase read error (Super Admin):", error);
        setIsLoading(false); // Agar tidak loading terus menerus jika gagal
      });
    } 
    // 🟢 LOGIKA ADMIN & INVESTOR: Tarik HANYA data akun yang dialokasikan kepadanya
      ...
      unsubUser = onValue(userRef, (snapshot) => {
        ...
        allowedIds.forEach(accId => {
          ...
          const unsubAcc = onValue(accRef, (accSnap) => {
            ...
            console.error(`Gagal akses akun ${accId}:`, err);
            setIsLoading(false);
          });
  ...
  // FIREBASE CLOUD NODE CONNECTION (TERMINAL)
  ...
  useEffect(() => {
    ...
    const watchdog = setInterval(() => {
      ...
      if (lastLogTimeRef.current > 0 && (now - lastLogTimeRef.current > 25000)) {
        setWsConnected(false); setRobotState('SYSTEM_ERROR');
      }
    }, 5000);
  ...
  const realDailyHistory = Object.keys(snapshots)
    ...
    .map(ts => {
      ...
      return { id: ts, date: dayNames[dateObj.getDay()], profit: dayData.daily_profit || 0, growth: dayData.daily_growth_percent || 0, lot: dayData.daily_lots || 0 };
    });
  ...
  // ==========================================
  if (isAuthLoading) return <div className="flex justify-center items-center h-screen bg-[#030712] font-mono text-blue-500 animate-pulse text-sm tracking-widest uppercase">Verifying Security Clearance...</div>;
  ...
  // TAMPILAN JIKA INVESTOR/ADMIN BELUM DIBERI AKSES
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 max-w-7xl mx-auto font-sans transition-colors duration-300">
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        @keyframes god-shine { 0% { transform: translateY(-100%); opacity: 0; } 20% { opacity: 1; } 80% { opacity: 1; } 100% { transform: translateY(100%); opacity: 0; } }
      ...
      <div className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-3xl p-6 md:p-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 shadow-sm relative overflow-hidden">
        ...
        <div className="z-10 w-full lg:w-auto flex flex-col items-start lg:items-end gap-4 bg-[var(--muted)]/50 p-4 rounded-2xl border border-[var(--card-border)]">
          ...
          <p className="text-[10px] text-[var(--muted-foreground)] font-medium flex items-center gap-1 mt-1"><Clock size={10} className="text-[var(--primary)]"/> {t.update} {formatTimeGMT8(liveData.last_update)}</p>
        </div>
      </div>
